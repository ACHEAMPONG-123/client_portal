...
import { submitDeliverableVersionAction, getAgencyTaskBoardData } from "@/app/actions/deliverable-actions";
...
export default function AgencyTaskBoardPage() {
  ...
  const loadBoardData = async () => {
    setLoading(true);
    try {
      const res = await getAgencyTaskBoardData();
      setData(res);
      if (res.projects && res.projects.length > 0 && !projectId) {
  ...
  const underReviewDeliverables = deliverables.filter((d: any) => d.status === "SUBMITTED" || d.status === "UNDER_REVIEW");
  const clientVisibleDeliverables = deliverables.filter((d: any) => d.clientVisible || d.status === "APPROVED");

  return (
    <div className="flex min-h-screen bg-[#F2F3F4]">
      <Sidebar userRole={userSession?.roleName || "DESIGNER"} tenantName="Agency Workspace" />

      <div className="flex-1 flex flex-col min-w-0">
        <Header user={{ name: userSession?.name || "Elena Rostova", email: userSession?.email || "designer@agency.com", roleName: userSession?.roleName || "DESIGNER" }} />
            ...
            <Card>
              <CardHeader className="pb-3 border-b border-[#F1F5F9]">
                <div className="flex items-center justify-between">
                  ...
                  <Badge variant="inProgress">{inProgressTasks.length} Active</Badge>
                </div>
              </CardHeader>
              <CardContent className="pt-4 space-y-3">
                {loading ? (
                  <p className="text-xs text-[#94A3B8] text-center py-4">Loading tasks...</p>
                ) : inProgressTasks.length === 0 ? (
                  <p className="text-xs text-[#94A3B8] text-center py-4">No in-progress tasks.</p>
                ) : (
                  inProgressTasks.map((task: any) => (
                    <div key={task.id} className="p-4 rounded-xl border border-[#E2E8F0] bg-[#F8FAFC] space-y-2">
                      <div className="flex items-center justify-between text-xs">
                        <span className="font-bold text-[#138808]">{task.tenant?.name || task.project?.name}</span>
                        <Badge variant="urgent">{task.priority}</Badge>
                      </div>
                      <h4 className="text-xs font-bold text-[#0F172A]">{task.title}</h4>
                      <p className="text-[11px] text-[#64748B]">{task.description || "Task in progress."}</p>
            ...
            <Card>
              <CardHeader className="pb-3 border-b border-[#F1F5F9]">
                <div className="flex items-center justify-between">
                  ...
                  <Badge variant="pending">{underReviewDeliverables.length} Submitted</Badge>
                </div>
              </CardHeader>
              <CardContent className="pt-4 space-y-3">
                {loading ? (
                  <p className="text-xs text-[#94A3B8] text-center py-4">Loading deliverables...</p>
                ) : underReviewDeliverables.length === 0 ? (
                  <p className="text-xs text-[#94A3B8] text-center py-4">No deliverables under review.</p>
                ) : (
                  underReviewDeliverables.map((del: any) => (
                    <div key={del.id} className="p-4 rounded-xl border border-[#E2E8F0] bg-white space-y-2">
                      <div className="flex items-center justify-between text-xs">
                        <span className="font-bold text-[#138808]">{del.tenant?.name || del.project?.name}</span>
                        <Badge variant="gold">{del.currentVersion}</Badge>
                      </div>
                      <h4 className="text-xs font-bold text-[#0F172A]">{del.name}</h4>
                      <p className="text-[11px] text-[#64748B]">{del.clientDescription || del.internalNotes || "Deliverable submission."}</p>
                      {del.externalDriveLink && (
                        <a
                          href={del.externalDriveLink}
                          target="_blank"
                          rel="noreferrer"
            ...
            <Card>
              <CardHeader className="pb-3 border-b border-[#F1F5F9]">
                <div className="flex items-center justify-between">
                  ...
                  <Badge variant="completed">{clientVisibleDeliverables.length} Active</Badge>
                </div>
              </CardHeader>
              <CardContent className="pt-4 space-y-3">
                {loading ? (
                  <p className="text-xs text-[#94A3B8] text-center py-4">Loading completed...</p>
                ) : clientVisibleDeliverables.length === 0 ? (
                  <p className="text-xs text-[#94A3B8] text-center py-4">No client-visible deliverables.</p>
                ) : (
                  clientVisibleDeliverables.map((del: any) => (
                    <div key={del.id} className="p-4 rounded-xl border border-[#A5D6A7] bg-[#E8F5E9] space-y-2">
                      <div className="flex items-center justify-between text-xs">
                        <span className="font-bold text-[#138808]">{del.tenant?.name || del.project?.name}</span>
                        <Badge variant="active">{del.approvalStatus}</Badge>
                      </div>
                      <h4 className="text-xs font-bold text-[#0F172A]">{del.name}</h4>
                      <p className="text-[11px] text-[#475569]">{del.clientDescription || "Available in client portal."}</p>
                    </div>
                  ))
                )}
              </CardContent>
